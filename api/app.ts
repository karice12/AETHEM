...
const checkSubscription = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    const expiresAt = userData.expiresAt?.toDate
      ...
      : null;
    const isSubscribed =
      userData.subscriptionStatus === "active" && expiresAt && expiresAt > new Date();

    if (!isSubscribed) {
      return res.status(403).json({
        error: "Assinatura Inativa: Upgrade necessário para acesso total ao Kernel.",
      });
    }
  ...
  try {
    ...
    if (!stripe) {
      ...
      return res.status(500).json({
        error:
          "Funcionalidade de pagamento requer a configuração da chave STRIPE_SECRET_KEY no ambiente.",
      });
    }
    ...
    const amount = plan === "monthly" ? 4990 : 29990;
    const title =
      plan === "monthly" ? "AETHEM - Elite Monthly" : "AETHEM - Overlord Yearly";
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              name: title,
              description: `Acesso Neural AETHEM - Plano ${
